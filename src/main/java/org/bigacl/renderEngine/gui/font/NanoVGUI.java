...
public class NanoVGUI {
   ...
   */
  public void drawText(String text, Vector2f position, float size, Vector3f color){
  ...
  public void drawRoundedRect(float x, float y, float w, float h, float radius, float r, float g, float b, float a) {
    try (MemoryStack stack = MemoryStack.stackPush()) {
      NVGColor color = NVGColor.malloc(stack);
      nvgRGBAf(r, g, b, a, color);
